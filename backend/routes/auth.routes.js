...
router.put('/profile', protect, upload.single('profilePic'), async (req, res) => {
    try {
        ...
        // Handle profile picture - either upload or avatar selection
        if (req.file) {
            // File was uploaded
            user.profilePic = req.file.filename;
            user.avatar = ''; // clear cartoon avatar when uploading image
        } 
        if (req.body.avatar) {
            // Avatar was selected (it's a path like '/avatars/avatar-2.jpg')
            user.avatar = req.body.avatar;
            user.profilePic = ''; // clear uploaded image when selecting avatar
        }
