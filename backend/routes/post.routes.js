...
router.get('/', async (req, res) => {
    try {
        const posts = await Post.find({ status: 'published' })
            .populate('author', 'name profilePic avatar')
            .sort({ createdAt: -1 });
        res.json(posts);
    } catch (err) {
...
router.get('/:id', async (req, res) => {
    try {
        const post = await Post.findById(req.params.id).populate('author', 'name profilePic avatar');
...
router.post('/', protect, memberOrAdmin, upload.single('image'), async (req, res) => {
    try {
        ...
        await post.populate('author', 'name profilePic avatar');
